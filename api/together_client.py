"""
Client for the Together API for LLM processing.
"""
...
class TogetherClient:
    ...
    def __init__(self, api_key: Optional[str] = None):
        ...
        self.rate_limit = 600  # Tier 1: 600 requests per minute
    ...
    async def _adaptive_rate_limit(self):
        ...
        # If approaching rate limit (>80% of limit), add adaptive delay
        if len(self.recent_requests) > 0.8 * self.rate_limit:
            # Calculate a delay that adapts to how close we are to the limit
            usage_ratio = len(self.recent_requests) / self.rate_limit
            adaptive_delay = max(0.1, usage_ratio * 2 - 1)  # Min 0.1s, scales up as we approach limit
    ...
    async def complete_async(
        ...
        attempt = 0
        base_delay = TOGETHER_RETRY_BASE_DELAY
        ...
        while True:  # Infinite retry loop
            try:
                ...
                if is_rate_limit or is_server_overload:
                    # Log the error
                    if is_rate_limit:
                        logging.warning(f"Rate limit error: '{str(e)[:100]}...")
                    else:
                        logging.warning(f"Server overload error: '{str(e)[:100]}...")
                    ...
                    logging.warning(f"API error. Retrying in {delay:.2f} seconds (attempt {attempt+1}/∞)")
                    await asyncio.sleep(delay)
                    attempt += 1
                else:
                    # For other errors, also retry (with a warning)
                    logging.warning(f"Error calling Together API: {str(e)}. Will retry...")
