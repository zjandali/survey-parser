"""
Client for Gemini-2.5-Pro via OpenRouter API using OpenAI client.
"""
...
class GeminiClient:
    ...
    def __init__(self, api_key: Optional[str] = None):
        ...
        if not self.api_key:
            raise ValueError("OpenRouter API key is required for Gemini-2.5-Pro access")
    ...
    def _clean_json_content(self, text: str) -> str:
        """
        Clean and format JSON content from Gemini model output
        ...
        if json_match:
            ...
            try:
                ...
                try:
                    ...
                    logging.warning("Unable to fix JSON format in Gemini response")
                    return text
        return text
    ...
    async def complete_async(
        ...
        Args:
            ...
            model: The model to use (default: google/gemini-2.5-pro-preview-03-25)
        ...
        while True:  # Infinite retry loop
            try:
                ...
                if "Survey Provider:" in prompt or "ALLOWED RESPONSES - Choose exactly ONE of these:" in prompt:
                    logging.info(f"Gemini raw provider response: '{raw_content}'")
                ...
                if is_rate_limit or is_server_overload:
                    # Log the error
                    if is_rate_limit:
                        logging.warning(f"Gemini rate limit error: '{str(e)[:100]}...")
                    else:
                        logging.warning(f"Gemini server overload error: '{str(e)[:100]}...")
                    ...
                    logging.warning(f"Gemini API error. Retrying in {delay:.2f} seconds (attempt {attempt+1}/∞)")
                    await asyncio.sleep(delay)
                    attempt += 1
                else:
                    # For other errors, also retry (with a warning)
                    logging.warning(f"Error calling Gemini API: {str(e)}. Will retry...")
...
"""
    ...
    def get_classifier_prompt(self, text: str, file_extension: str, is_chunk: bool = False, chunk_info: str = "") -> str:
        ...
        """
        # Modified prompt specifically designed for Gemini to handle correctly
