"""
Client for OpenRouter API access using OpenAI client for LLM processing.
"""
...
class OpenRouterClient:
    """Client for OpenRouter API using OpenAI client"""
    ...
    async def _adaptive_rate_limit(self):
        ...
        if len(self.recent_requests) > 0.7 * self.rate_limit:
            ...
            adaptive_delay = max(0.2, usage_ratio * 3 - 1.5)  # More conservative delay than Together
    ...
    async def complete_async(
        ...
        Args:
            ...
            model: The model to use (default: deepseek/deepseek-chat)
        ...
        while True:  # Infinite retry loop
            try:
                ...
                if is_rate_limit or is_server_overload:
                    # Log the error
                    if is_rate_limit:
                        logging.warning(f"OpenRouter rate limit error: '{str(e)[:100]}...")
                    else:
                        logging.warning(f"OpenRouter server overload error: '{str(e)[:100]}...")
                    ...
                    logging.warning(f"OpenRouter API error. Retrying in {delay:.2f} seconds (attempt {attempt+1}/∞)")
                    await asyncio.sleep(delay)
                    attempt += 1
                else:
                    # For other errors, also retry (with a warning)
                    logging.warning(f"Error calling OpenRouter API: {str(e)}. Will retry...")
